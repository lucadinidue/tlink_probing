...
def prepare4conversion(sentence):
    ...
    for elem in token_list:
        token = elem.strip().split("\t")[0]
        token_id = elem.strip().split("\t")[1]
        sent_id = elem.strip().split("\t")[2]
        event_id = elem.strip().split("\t")[3]
#        timex = elem.strip().split("\t")[2]
        tlink = elem.strip().split("\t")[4]
        data.append((token_id, token, event_id))
...
def process_data(input_file):
    ...
    counter = -1
    outfile = "./TB_ENDense_col/" + "test.txt"
...
if __name__ == '__main__':
    ...
    inputtrain_dir = "/home/p281734/projects/tlink_probing/workspace/TB_ENDense_col/test/"
